...
- Write a route in Express.js that serves a `home.html` file. Create a new HTML file called `home.html` that contains an `<h1>` tag with the message "Welcome to ExpressJs Tutorial". Ensure that the home page is served to the client when the `/home` route is accessed. --done
...
process.env.port refers to the PORT variable defined in the environment.

In production (e.g., when deploying to Heroku, AWS, Azure, or Docker), the hosting provider often assigns a random or specific port dynamically.

Example: PORT=5432

|| 8081

If process.env.port is not defined (e.g., running locally without setting an environment variable), Node.js will use 8081 as a fallback.

This makes it easier to run your app both locally and in production without changing the code.
